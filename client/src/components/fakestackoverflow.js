  ...
  async componentDidMount() {
    ...
    if (this.props.x.pathname.includes("answers")) {
      const x = await axios.get(
        `http://localhost:8000/main/question/answer/${this.props.x.pathname.split(":")[3]}`
      );
      this.setState({
        results: x.data,
    ...
    if (this.props.x.pathname.includes("tagname")) {
      const list = [];
      axios.get("http://localhost:8000/main/question").then((res) => {
